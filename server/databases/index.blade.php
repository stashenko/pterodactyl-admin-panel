    ...
    @if($allowCreation && Gate::allows('create-database', $server))
        <div class="col-xs-12 col-sm-4">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Создать базу данных</h3>
                </div>
                @if($overLimit)
                    <div class="box-body">
                        <div class="alert alert-danger no-margin">
                            В настоящее время вы используете <strong>{{ count($databases) }}</strong> из <strong>{{ $server->database_limit ?? '∞' }}</strong> разрешенных баз данных.
                        </div>
                    </div>
                @else
                    <form action="{{ route('server.databases.new', $server->uuidShort) }}" method="POST">
                        ...
                        <div class="box-footer">
                            {!! csrf_field() !!}
                            <p class="text-muted small">Вы используете <strong>{{ count($databases) }}</strong> и <strong>{{ $server->database_limit ?? '&infin;' }}</strong> баз данных. Имя пользователя и пароль для этой базы данных будут генерироваться случайным образом после отправки формы.</p>
